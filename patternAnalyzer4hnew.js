 ...
 */
export async function getPatternData4h(a) {
  let startTime = Date.now() - (a + 1)*4 * 60 * 60 * 1000;
  ...
  while (startTime < endTime) {
    ...
    startTime = data[data.length - 1][0] + 4 * 60 * 60 * 1000;
  }
...
export async function getPatternString4h(a) {
  const { pattern } = await getPatternData4h(a);
  return pattern;
}
