import { getPatternData4h } from "./patternAnalyzer4hnew.js";
import { analyzePattern4h } from "./analyzer4hnew.js";

export async function findLongestPattern4h() {
  let a = 1;
  let statsHistory = [];

  while (true) {
    console.log(`\n⏳ Đang lấy pattern 4h với length = ${a}...`);

    const live = await getPatternData4h(a); // { pattern, closes, netChange, netChangePercent, trend }
    console.log(`📊 Pattern 4h: ${live.pattern}`);
    ...
    const result = analyzePattern4h(patternArray, live.trend);

    if (result.total === 0) {
      console.log(`❌ Pattern 4h này KHÔNG tìm thấy trong lịch sử`);
      break;
    }
